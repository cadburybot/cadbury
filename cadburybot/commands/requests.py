...
@app.command()
def create(name: str):
    authorization_header, base_url, port, protocol = ask_env_details_from_user()
...
def edit(name: str):
    try:
        ...
        authorization_header, base_url, port, protocol = ask_env_details_from_user(current_env)
...
def ask_env_details_from_user(env=None):
    ...
    port = typer.prompt("What is the port? (optional)", default=port)
    authorization_header = typer.prompt("What is the authorization header value? It looks something like this `Bearer "
                                        ...
                                        show_default=False)
    return authorization_header, base_url, port, protocol
